...
private void Click_BackList(object sender, System.EventArgs e)
{
    ...
    if (stralert != "")
    {
        Response.Redirect(stralert);
    }
    else
    {
        if (hycode == "") Response.Redirect("list_0.aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + mid + "&tid=" + tid + "&pid=" + pid);
        else Response.Redirect("list_1199.aspx?aid=" + StringUtility.StringToBase64("list") + "&hycode=" + StringUtility.StringToBase64(hycode) + "&mid=" + mid + "&tid=" + tid + "&pid=" + pid);
    } 
}
...
private void SetToolBar()
{
    ...
    {
        string stralert = Request.UrlReferrer.ToString();//前一页的地址，用来判断返回的列表地址
        if ((GetSESSION("extinfo") == "" && stralert.IndexOf("default.aspx") == -1))
        {
            SetSESSION("extinfo", stralert);
        }
    }
    //Response.Write(mid);
    //判断是否为开发用户
...
<td width="100%"><span id="companyname" runat="server" style="color:Black"></span></td>
