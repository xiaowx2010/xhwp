...
{	
	...
	if(h_status=="1")//已办理
	{
		h_fstr += " and (select count(1) from gmis_mo_29 where fld_29_3=fld_17_1)>0";
	...
	foreach (string[] arr in al)
	{
		if (arr[2] == sortid)
		{
			...
			h_sb_head.Append("<td style=\"padding-top:3px;\"  width=\"" + arr[1] + "\" ><a href=\"list_29.aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + mid + "&sortid=" + StringUtility.StringToBase64(arr[2]) + "&sortdir=" + sortdir + "\" class=\"listtitle\"  title=\"" + arr[0] + h_dirstr + "\">" + arr[0] + "</a>" + h_dirimg + "</td>\n");
			
		}
		else
		{
			h_sb_head.Append("<td width=\"" + arr[1] + "\" ><a href=\"list_29.aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + mid + "&sortid=" + StringUtility.StringToBase64(arr[2]) + "&sortdir=" + sortdir + "\" class=\"listtitle\"  title=\"" + arr[0] + "\">" + arr[0] + "</a></td>\n");
		}
	}
	...
	//获取所有已经许可的污水污染物
	p_wrwdt = db.GetDataTable(db.ConnStr,"select fld_29_3,fld_1206_1 from gmis_mo_29 b left outer join gmis_mo_1206 a on b.fld_29_2=a.mocode ");
...
{	
	...
	{
		DataRow[] h_wrwdr = p_wrwdt.Select("fld_29_3='"+dr["fld_17_1"].ToString()+"'","");
		if (h_wrwdr.Length>0)
		{
			foreach (DataRow ddr in h_wrwdr)
			{
				dr["wrwname"] += ddr["fld_1206_1"].ToString()+",";
			}
...
private void CheckSelect()
{
	...
	Response.Redirect("list_29.aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + mid + "&navindex=0&sortid=" + sortid + "&sortdir=" + sortdir + "");
}
</script>
