...
private void Page_Load(object sender, System.EventArgs e)
{
	...
	dtable = "gmis_mo_1228";
    flds = new string[] { "fld_1228_1", "fld_1228_2", "fld_1228_3", "fld_1228_4", "fld_1228_5", "fld_1228_6"};
    types = "000010";	
	
	if (!IsPostBack)
	{
		//查询模板
		BindListControl(edition, "mocode", "fld_1228_1", "select mocode,fld_1228_1 from gmis_mo_1228 order by mocode", "无模版");
		SetFilter(edition, "0");

		//项目模块
		DataTable h_mdt = db.GetDataTable(db.ConnStr,"select modulecode,(REPLICATE('　',modulelevel-2)+modulename) as modulename from gmis_module where moduleindex like '%'+(select moduleindex from gmis_module where modulecode=17)+'%' and modulestate=0 and exists (SELECT 1 FROM sysobjects WHERE xtype = 'U' AND OBJECTPROPERTY (id, 'IsMSShipped') = 0 and id=object_id ('gmis_mo_'+Cast(modulecode as varchar(50)))) order by moduleindex");
...
private void GetDefaultField()
{
	if (guangye.GetControlValue(module) != "")
	{
		...
		guangye.SetControlValue(fld_1228_4,h_field);
	}
}
//模块筛选
...
{	
	...
	{		
		string h_modulecode =  GetOneFieldValue("select fld_1228_5 from gmis_mo_1228 where mocode = "+guangye.GetControlValue(edition));//模块ID
		if (h_modulecode != "")
		{
	...
	{
		Response.Redirect("list_1228.aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + mid + "&navindex=" + navindex + "");
	}
}
/***************************************************************
函数名：BindEdition()
...
private void BindEdition()
{
    ...
    s_fstr.InnerHtml = "";
    i_fstr.InnerHtml = "";
	string[] h_sfield = guangye.GetControlValue(fld_1228_6).Trim('|').Split('|');
	...
	//将排序加入到显示框中
	string[] h_ifield = guangye.GetControlValue(fld_1228_3).Trim('|').Split('|');
...
{	
	...
	else 
	{
		if (guangye.GetControlValue(newname) != "")
		{
			...
			DataTable h_checkdt = db.GetDataTable(db.ConnStr,"select mocode from gmis_mo_1228 where fld_1232_1='"+guangye.GetControlValue(newname)+"'");
			...
			{
				guangye.SetControlValue(fld_1228_6,guangye.GetControlValue(fld_1228_6).Trim('|'));//去掉前后“|”
			
				fld_1228_2.Value=StringUtility.StringToBase64(CheckSearchFilter(guangye.GetControlValue(fld_1228_6)));//获取查询条件

				guangye.SetControlValue(fld_1228_1,guangye.GetControlValue(newname));

				guangye.SetControlValue(fld_1228_5,guangye.GetControlValue(module));
	...
	//查询模板
	DataTable h_edt = db.GetDataTable(db.ConnStr,"select top 1 mocode from gmis_mo_1228 order by mocode desc");	
	BindListControl(edition, "mocode", "fld_1228_1", "select mocode,fld_1228_1 from gmis_mo_1228 order by mocode", "无模版");
...
{	
	...
	{
		guangye.SetControlValue(fld_1228_5,guangye.GetControlValue(module));
		SetSESSION("filter",fld_1228_2.Value+"#"+guangye.GetControlValue(fld_1228_3)+"#"+guangye.GetControlValue(fld_1228_4)+"#"+guangye.GetControlValue(fld_1228_5)+"#"+guangye.GetControlValue(fld_1228_6));
...
function S_Add_Click()
{
	...
	if (document.getElementById("s_fstr").innerHTML != "" && (window.form1.and.checked || window.form1.or.checked))
	{
		...
		{
			window.form1.fld_1228_6.value += "|并且";
			document.getElementById("s_fstr").innerHTML += " <b>并且</b> ";
		}
		else
		{
			window.form1.fld_1228_6.value += "|或者";
	...
	{
		window.form1.fld_1228_6.value += "|"+window.form1.s_field.value+";"+window.form1.compare.options[window.form1.compare.selectedIndex].innerHTML+";"+window.form1.comparekey.value.replace("\'","").replace(",","，");
...
function S_Cancel_Click()
{
	if(document.getElementById("s_fstr").innerHTML.indexOf("[")>-1)
	{
		...
		window.form1.fld_1228_6.value = window.form1.fld_1228_6.value.substring(0,window.form1.fld_1228_6.value.lastIndexOf("|")-3);
	}
	else
	{
		document.getElementById("s_fstr").innerHTML = "";
		window.form1.fld_1228_6.value = "";
	}
}
/***************************************************************
函数名：S_Clear_Click()
...
function I_Add_Click()
{
	...
	{
		window.form1.fld_1228_3.value += window.form1.i_field.value+";"+window.form1.updown.value;
...
function I_Cancel_Click()
{
	if(document.getElementById("i_fstr").innerHTML.indexOf("[")>-1)
	{
		...
		window.form1.fld_1228_3.value = window.form1.fld_1228_3.value.substring(0,window.form1.fld_1228_3.value.lastIndexOf(","));
	}
	else
	{
...
<body style="padding: 10px; text-align: center">
    ...
    <!--选项卡-->
	<!--模版名称-->
	<input id="fld_1228_1" runat="server" type="hidden" />
	<!--模版名称-->
	<!--查询条件-->
	<input id="fld_1228_2" runat="server" type="hidden" />
	<!--查询条件-->
	<!--排序条件-->
	<input id="fld_1228_3" runat="server" type="hidden" />
	<!--排序条件-->
	<!--显示字段-->
	<input id="fld_1228_4" runat="server" type="hidden" />
	<!--显示字段-->
	<!--模块ID-->
	<input id="fld_1228_5" runat="server" type="hidden" />
	<!--模块ID-->
	<!--查询条件字段-->
	<input id="fld_1228_6" runat="server" type="hidden" />
	<!--查询条件字段-->
                          ...
                          <tr>
                            ...
                            <td class="btn_M_bg"><table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                  <td valign="bottom" class="btn_text" >
								  <!--设置显示字段-->
								  <G:Button id="btn_selectfield" type="toolbar" mode="on" icon="" text="设置显示字段" Url="javascript:var v_open = open('pop_1228_0.aspx?mid='+form1.module.value)"  runat=server></G:Button>
								  <!--设置显示字段-->
								  </td>
                                </tr>
                            </table></td>
                          ...
                          <tr>
                            ...
                            <td class="btn_M_bg"><table border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                  <td valign="bottom" class="btn_text">
								  <!--预览表头-->
								  <G:Button id="btn_showfield" type="toolbar" mode="on" icon="" text="预览表头" Url="javascript:if(form1.fld_1228_4.value!=''){var v_open = open('pop_1228_2.aspx?field='+form1.fld_1228_4.value);}else{alert('请先设置显示字段！')}" runat=server ></G:Button>
								  <!--预览表头-->
								  </td>
                                </tr>
                            </table></td>
