...
<script runat="server" language="C#">
    ...
    private void BindDataList()
    {
        ...
        al.Add(new string[] { "2009年数值（"+h_unit+"）", "18%", h_newFieldCode });
        al.Add(new string[] { "2007年数值（"+h_unit+"）", "18%", h_oldFieldCode });
        ...
        //设置列表控件显示行数
        //设置列表控件数据源    
       list.SqlStr = "select distinct '' as orgcode,fld_1280_2,'"+h_fieldName+"' as h_fieldName,isnull("+h_newFieldCode+",0) as h_newFieldCode,"+h_oldFieldCode+" as h_oldFieldCode,fld_1280_41,fld_1280_1 from "+tablename+"  where 1=1 " + h_fstr + " " + orderstr + " ";
		...
		if(h_fieldName != "排水去向")
		{
			...
			AmountMes.InnerHtml="合计 07年数据："+h_old.ToString()+"（"+h_unit+"）; 09年数据："+h_new.ToString()+"（"+h_unit+"）。<font color=red>同比增长"+(h_new-h_old)+"（"+h_unit+"）</font>,<font color=red>约"+((h_new-h_old)*100/h_old).ToString("f2")+"%</font>";
		}
		else
		{
			AmountMes.InnerHtml="&nbsp;";
