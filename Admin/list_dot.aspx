...
{    
	...
	{	
		...
		BindListControl(selecttype,"typecode","typename","select typecode,typename from gmis_type where typestate='启用' and typekind='数据点'"+h_tstr,"所有图层");	
    ...
    list.SqlStr = "declare @allcount int;select @allcount=count(1) from " + tablename + " where FeatureKind=1 " + h_fstr + h_tstr + h_astr + ";select @allcount as allcount,'" + StringUtility.StringToBase64("view") + "'," + mid + "," + tid + ",Featurecode," + navindex + ",Featurename,t.typename,a.areaname,case when Featurestate = 0 then '启用' when Featurestate = -1 then '禁用' when Featurestate = 1 then '隐藏' else '&nbsp;' end as Featurestate,'' as btnstr,f.typecode from " + tablename + " where FeatureKind=1 " + h_fstr + h_tstr + h_astr + " and Featurecode not in (select top " + fromcount + " Featurecode from " + tablename + " where FeatureKind=1 " + h_fstr + h_tstr + h_astr + orderbystr + ")  and Featurecode in (select top " + (fromcount + pagesize) + " Featurecode from " + tablename + " where FeatureKind=1 " + h_fstr + h_tstr + h_astr + orderbystr + ")" + orderbystr;
    list.Rows = pagesize;
}
...
{	
	...
	Response.Redirect("list_dot.aspx?aid=list&mid="+mid);
}
</script>
...
<td>
	<table id="tb_6" visible="false" runat="server">
		<tr>
			...
			<td><G:Button id="search_btn" type="toolbar" mode="on" icon="" text="查询" Style="padding-top:2px;" runat="server"></G:Button>
...
<G:ListTable ID="list" Rows="20" IsProPage="true" runat="server">
	<G:Template id="listtemphead" type="head" runat="server">
		<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="righttableline">
			<tr class="tr_listtitle" align="left">
				<td>&nbsp;</td> 
				<td width="45%" align="left">数据点名称</td>
				<td width="24%" align="left">所属数据层</td>
				<td width="15%" align="left">所属区域</td>
	...
	<G:NavStyle5 ID="NavStyle" PageUrl="list_dot.aspx" runat="server"></G:NavStyle5>
	</G:ListTable> 
<!--动态生成html,结束-->
</form>
</body>
