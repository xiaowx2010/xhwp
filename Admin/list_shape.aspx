...
{         
	...
	{	
		...
		BindListControl(selecttype,"typecode","typename","select typecode,typename from gmis_type where typestate='启用' and typekind='图形'"+h_tstr,"所有图层");				
    ...
    string orderbystr = " order by f.typecode";//获取排序条件，假设方法已定义；
    ...
    list.SqlStr = "declare @allcount int;select @allcount=count(1) from " + tablename + " where (featurekind=2 or typekind='图形') " + h_fstr + h_tstr + h_astr + ";select @allcount as allcount,'" + StringUtility.StringToBase64("view") + "'," + mid + "," + tid + ",Featurecode," + navindex + ",Featurename,t.typename,a.areaname,case when Featurestate = 0 then '启用' when Featurestate = -1 then '禁用' when Featurestate = 1 then '隐藏' else '&nbsp;' end as Featurestate,'' as btnstr,f.typecode from " + tablename + " where (featurekind=2 or typekind='图形') " + h_fstr + h_tstr + h_astr + " and f.typecode not in (select top " + fromcount + " f.typecode from " + tablename + " where (featurekind=2 or typekind='图形') " + h_fstr + h_tstr + h_astr + orderbystr + ")  and f.typecode in (select top " + (fromcount + pagesize) + " f.typecode from " + tablename + " where (featurekind=2 or typekind='图形') " + h_fstr + h_tstr + h_astr + orderbystr + ")" + orderbystr;
    list.Rows = pagesize;
}
	...
	{			
		...
		Response.Redirect("list_shape.aspx?aid=list&mid="+mid);
	}
</script>
...
<td>
	<table id="tb_6" visible="false" runat="server">
		<tr>
			...
			<td><G:Button id="search_btn" type="toolbar" mode="on" icon="" Style="padding-top:2px;" text="查询" runat="server"></G:Button></td>
		</tr>
	</table>
</td>
...
<G:ListTable ID="list" Rows="20" IsProPage=true runat="server">
	<G:Template id="listtemphead" type="head" runat="server">
		...
		<tr class="tr_listtitle" align="left">
			<td>&nbsp;</td> 
			<td width="45%" align="left">图形名称</td>
			<td width="24%" align="left">简称</td>
	...
	<G:NavStyle5 ID="NavStyle" PageUrl="list_14.aspx" runat="server"></G:NavStyle5>
	</G:ListTable> 
<!--动态生成html,结束-->
</form>
</body>
