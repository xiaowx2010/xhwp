...
{	
	...
	string h_orderstr = " order by fld_53_1 asc";//排序条件;
	...
	al.Add(new string[] { "单位代码", "15%", "fld_53_3" });
	al.Add(new string[] { "单位名称", "22%", "fld_53_4" });
	al.Add(new string[] { "排放口代码", "15%", "fld_53_1" });
	al.Add(new string[] { "排放口名称", "25%", "fld_53_2" });
	...
	h_fsb.Append("'" + StringUtility.StringToBase64("view") + "'," + mid + ",mocode,fld_53_3,fld_53_4,fld_53_1,fld_53_2,'' as wrw,'' as btnstr,fld_53_11,(select mocode from gmis_mo_17 where fld_17_1=fld_53_3) as mcode,fld_53_9");
	...
	foreach (string[] arr in al)
	{
		if (arr[2] == sortid)
		{
			...
			h_sb_head.Append("<td style=\"padding-top:3px;\"  width=\"" + arr[1] + "\" ><a href=\"list_53.aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + mid + "&sortid=" + StringUtility.StringToBase64(arr[2]) + "&sortdir=" + sortdir + "\" class=\"listtitle\"  title=\"" + arr[0] + h_dirstr + "\">" + arr[0] + "</a>" + h_dirimg + "</td>\n");
			
		}
		else
		{
			h_sb_head.Append("<td width=\"" + arr[1] + "\" ><a href=\"list_53.aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + mid + "&sortid=" + StringUtility.StringToBase64(arr[2]) + "&sortdir=" + sortdir + "\" class=\"listtitle\"  title=\"" + arr[0] + "\">" + arr[0] + "</a></td>\n");
		}
	}
	...
	{
		h_fstr += " and fld_53_6 = "+guangye.GetControlValue(select_type)+"";
	}
	
	//添加字段筛选条件
	if (guangye.GetControlValue(key) != "" && guangye.GetControlValue(select_field) != "" && guangye.GetControlValue(select_field) != "0")
	{
		string h_field = (guangye.GetControlValue(select_field)=="1")?"fld_53_3":"fld_53_4";
	...
	p_wrwdt = db.GetDataTable(db.ConnStr,"select mocode,(select fld_1205_1 from gmis_mo_1205 where mocode = fld_21_2) as wrwname,fld_21_3 from gmis_mo_21");//获取所有注册已处理污染物
}
...
{	//处理当前页数据
	...
	if(p_wrwdt != null && p_wrwdt.Rows.Count>0 && dr["fld_53_11"].ToString().Trim(';')!="")
	{
		...
		if (h_code.Length>0)
		{
			...
			{
				DataRow[] h_wrwdr = p_wrwdt.Select("mocode="+h_code[i]+" and fld_21_3 = '"+dr["fld_53_3"].ToString()+"'");
				if(h_wrwdr.Length>0)
				{					
	...
	dr["btnstr"] += "<a href=\"edit_19.aspx?aid=" + StringUtility.StringToBase64("edit") + "&mid=" + mid + "&pfkcode=" + StringUtility.StringToBase64(dr["fld_53_1"].ToString()) + "&dwcode=" + StringUtility.StringToBase64(dr["fld_53_3"].ToString()) + "&navindex=" + navindex + "\"><img src=\"images/icons/TB51.gif\" title=\"新增月报表\" align=\"absmiddle\" border=\"0\"></a>";//新增月报表
	
	
	if(dr["fld_53_9"].ToString()=="True")//判断是否监测
	{
		dr["btnstr"] += "<a href=\"edit_18.aspx?aid=" + StringUtility.StringToBase64("edit") + "&mid=" + mid + "&id=" + dr["mocode"].ToString() + "&navindex=" + navindex + "\"><img src=\"images/icons/TB52.gif\" title=\"编辑污水排放口监测\" align=\"absmiddle\" border=\"0\"></a>";//编辑废水排放口监测
	}
...
private void CheckSelect()
{
	...
	Response.Redirect("list_53.aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + mid + "&navindex=0&sortid=" + sortid + "&sortdir=" + sortdir + "");
}
</script>
...
<body style="padding: 10px; text-align: center">
    <form id="form1" runat="server">
		...
		<!--排放口类型-->
		<td nowrap >
		   排放口类型：<asp:DropDownList ID="select_type" AutoPostBack="true" OnSelectedIndexChanged="Change_Type" runat="server" >
			</asp:DropDownList>
		</td>
		<!--排放口类型-->
