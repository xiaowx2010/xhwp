...
{
    string stralert = GetSESSION("extinfo");
    SetSESSION("extinfo","");
    if (stralert != "")
    {
        if (stralert != "1199")
        {
            Response.Redirect("getpage.aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + stralert + "&tid=" + tid + "&pid=" + pid + "&navindex=" + navindex);
            //Response.Write("stralert!=1199");
        }
        else
        {
            Response.Redirect("list_" + stralert + ".aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + stralert + "&tid=" + tid + "&pid=" + pid + "&hycode=" + GetSESSION("hycode") + "&navindex=" + navindex);
        }
    }
    else
    {
        ...
        {
            Response.Redirect("list_" + stralert + ".aspx?aid=" + StringUtility.StringToBase64("list") + "&mid=" + mid + "&tid=" + tid + "&pid=" + pid + "&hycode=" + GetSESSION("hycode") + "&navindex=" + navindex);
            //Response.Write("stralert!=1199");
        }
    }
...
private void SetToolBar()
{
    ...
    //Response.Write(GetQueryString("hycode", "0"));
    //Response.Write("SetSESSION(\"hycode\", "+GetQueryString("+hycode+", "0")+")");
    //if (GetQueryString("hycode", "0") != "0")
    //{
    //    SetSESSION("hycode", GetQueryString("hycode", "0"));
    //}
    navindex = GetQueryString("navindex", "0");
    string qiyexinxibiao = GetQueryString("qiyexinxibiao", "0");
    if (qiyexinxibiao == "1199")
    {
        if (GetQueryString("hycode", "0") != "0")
        {
            SetSESSION("hycode", GetQueryString("hycode", "0"));
        }
        SetSESSION("extinfo", "1199");
        //SetSESSION("hycode", hycode);
        SetSESSION("qiyexinxibiao", "1199");
    }
    else
    {
        if (GetSESSION("extinfo") == "21" || GetSESSION("extinfo") == "")
        {
            SetSESSION("extinfo", mid);
            SetSESSION("qiyexinxibiao", "");
        }
    
        //Response.Write(GetSESSION("extinfo"));
        if (GetSESSION("extinfo") == "1199" && mid == "1199")
        {

            //Response.Write("=" + hycode);
    ...
    //Response.Write(GetSESSION("extinfo")+"<br>");
        //Response.Write("SetSESSION(\"hycode\", "+hycode+")");
    //Response.Write(mid);
    //判断是否为开发用户
