...
private void Page_Load(object sender, System.EventArgs e)
{
	...
	p_sonmua = ";" + GetModuleActions("29").Trim(';') + ";";//子模块权限
	
	/*模块编辑设置*/
	dtable = "gmis_mo_29";
	filter = " mocode=" + pid;
	flds = new string[] { "fld_29_1", "fld_29_2", "fld_29_3" };
	types = "110";
	/*模块编辑设置*/	
	...
	if(!IsPostBack)
	{
		...
		BindListControl(fld_29_2, "mocode", "fld_1206_1", "select mocode,fld_1206_1 from gmis_mo_1206 order by fld_1206_3,fld_1206_1","-请选择-");//绑定污染物名称 
		SetFilter(fld_29_2, "0");

		if(pid != "0")
		{			
			guangye.BindData(dtable, filter, flds);			
		}
		if (guangye.GetControlValue(fld_29_2) != "" && guangye.GetControlValue(fld_29_2) != "0")
		{
			DataTable h_tdt = db.GetDataTable(db.ConnStr,"select fld_1206_2,fld_1206_4,fld_1206_5 from gmis_mo_1206 where mocode = "+guangye.GetControlValue(fld_29_2)); 
			if (h_tdt.Rows.Count>0)
			{
				wrw_code.InnerText = h_tdt.Rows[0]["fld_1206_2"].ToString();
				wrw_nbz.InnerText = h_tdt.Rows[0]["fld_1206_4"].ToString();
				wrw_sbz.InnerText = h_tdt.Rows[0]["fld_1206_5"].ToString();
			}
		}
	}
	...
	if(dt.Rows.Count>0)
	{
		guangye.SetControlValue(fld_29_3,dt.Rows[0]["fld_17_1"].ToString()); 
	}
...
public override void BeforeOutput(DataTable dt, int rowi)
{
	...
	{
		dr["btnstr"] += "<a href=\"edit_29.aspx?aid=" + StringUtility.StringToBase64("edit") + "&mid=" + mid + "&pid=" + dr["mocode"].ToString() + "&id=" + id + "&navindex=" + navindex + "\"><img src=\"images/icons/tb02.gif\" title=\"编辑\" align=\"absmiddle\" border=\"0\"></a>";
	}
	...
	switch (idstr)
	{
		case "btn_saveson":
			...
			else
			{
				...
				DataTable h_wrwdt = db.GetDataTable(db.ConnStr,"select mocode from gmis_mo_29 where fld_29_3='"+guangye.GetControlValue(fld_29_3)+"' and fld_29_2="+guangye.GetControlValue(fld_29_2)+""+h_fstr);
			...
			else
			{
				...
				if (pid == "0")
				{
					exeinfo = db.UpdateTable(db.ConnStr, guangye.GetInsertSql(dtable, flds, types)+";SELECT @@IDENTITY AS 'MoCode';declare @mocode as int,@mid as int,@name as varchar(50);select @mocode=@@IDENTITY;select @mid=29;select @name=fld_17_3 from gmis_mo_17 where mocode="+id+";if not exists (select 1 from gmis_mo_1197 where ModuleCode=@mid and DataMoCode=@mocode) begin insert into gmis_mo_1197 (modulecode,datamocode,orgcode,orgname) values (@mid,@mocode,'" + guangye.GetControlValue(fld_29_3) + "',@name) end else begin update gmis_mo_1197 set orgcode='" + guangye.GetControlValue(fld_29_3) + "',orgname=@name where datamocode=@mocode and modulecode=29 end");
				}
				else
				{
					exeinfo = db.UpdateTable(db.ConnStr, guangye.GetUpdateSql(dtable, filter, flds, types)+";declare @name as varchar(50);select @name=fld_17_3 from gmis_mo_17 where fld_17_1='" + guangye.GetControlValue(fld_29_3) + "';if exists (select 1 from gmis_mo_1197 where ModuleCode=29 and DataMoCode="+pid+") begin update gmis_mo_1197 set orgcode='" + guangye.GetControlValue(fld_29_3) + "',orgname=@name where datamocode=" + pid + " and modulecode=29 end else begin insert into gmis_mo_1197 (modulecode,datamocode,orgcode,orgname) values (29,"+pid+",'" + guangye.GetControlValue(fld_29_3) + "',@name) end");
				}

				if (exeinfo.IndexOf("操作成功！")>-1)
				{
					Response.Redirect("edit_29.aspx?aid=" + StringUtility.StringToBase64("edit") + "&mid=" + mid + "&pid=0&id=" +id+ "");
				}
			}
			break;
		case "btn_deleteson":
			//删除的同时，一并删除gmis_mo_1197表（数据库关联表），并将gmis_mo_23(排放口表)数据表中，含有该污染物的记录，进行更新
			exeinfo = db.UpdateTable(db.ConnStr, "delete gmis_mo_29 where mocode=" + sonid.Value +"; delete gmis_mo_1197 where datamocode=" + sonid.Value + " and modulecode=29;update gmis_mo_23 set fld_23_11 = replace(';'+fld_23_11+';',';"+sonid.Value+";','') where ';'+fld_23_11+';' like '%;"+sonid.Value+";%'");
			if (exeinfo.IndexOf("操作成功！") > -1)
			{
				Response.Redirect("edit_29.aspx?aid=" + StringUtility.StringToBase64("edit") + "&mid=" + mid + "&pid=0&id=" +id+ "");
			}
		...
		case "btn_backlistson":
			...
			break;
		case "btn_addson":
			Response.Redirect("edit_29.aspx?aid="+StringUtility.StringToBase64("edit")+"&mid="+mid+"&navindex="+navindex+"&id="+id+"&tid="+tid+"&pid=0");
			break;
	}
...
private void Change_Type(object sender, System.EventArgs e)
{
	...
	{
		h_fstr = " and fld_1206_3="+guangye.GetControlValue(select_type)+"";
	}	
	BindListControl(fld_29_2, "mocode", "fld_1206_1", "select mocode,fld_1206_1 from gmis_mo_1206 where 1=1 "+h_fstr+" order by fld_1206_3,fld_1206_1","-请选择-");//绑定污染物名称
	SetFilter(fld_29_2, "0");
...
<body style="padding: 10px; text-align: center">
    ...
    <!--污染物列表-->
	<!--新增，编辑污染物-->
	<input id="fld_29_3" type="hidden" runat="server" />
	...
	<tr>
		...
		<td class="td_viewcontent_content" style="width:35%"><asp:DropDownList ID="fld_29_2" AutoPostBack="true" OnSelectedIndexChanged="Change_Name" Style="width:95%;" runat="server"></asp:DropDownList></td>
	</tr>
	<tr>
		...
		<td width="100" align="right"  class="td_viewcontent_title">浓度执行标准:</td>
		<td class="td_viewcontent_content" style="width:35%"><lable id="wrw_nbz" runat=server></lable>&nbsp;</td>
	</tr>	
	<tr>
		<td width="100" align="right"  class="td_viewcontent_title">速率执行标准:</td>
		<td class="td_viewcontent_content" style="width:35%"><lable id="wrw_sbz" runat=server></lable>&nbsp;</td>
		<td width="100" align="right"  class="td_viewcontent_title">许可排放量:</td>
		<td class="td_viewcontent_content" style="width:35%"><input id="fld_29_1" class="boxbline" style="width:95%;"  maxlength="50" runat="server" ></td>
	</tr>
	</table>
    <!--新增，编辑污染物-->
    <!--动态生成html,结束-->
